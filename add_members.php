...
$i=0;
    ...
    echo "<script>setTimeout(\"location.href = 'adminlogin.php';\",1500);</script>";    
        ...
        echo $flat,$owner,$members,$username,$pass,$area,$twowheeler,$fourwheeler,$dues;
...
if ($i==0){
            echo "Enter";    
            $INSERT1 = "INSERT into housingmembers(FlatAddress, FlatOwner, No_of_House_Members, Username, Password) values(?, ?, ?,?,?)";
            $stmt = $con->prepare($INSERT1);
            $stmt->bind_param("ssdss", $flat, $owner, $members, $username, $pass);
            $stmt->execute();
            $stmt->close();
